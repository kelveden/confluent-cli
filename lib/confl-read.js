...
const action = (config) => (options) => {
    ...
    process.stdin
        .pipe(through2.obj(function (messageBytes, enc, callback) {
            const buf = Buffer.from(messageBytes.toString(), "hex");

            avroDeserializer.deserialize(buf, config)
                .then((body) => {
                    this.push(body);
                    callback();
                })
...
const command = (program, config) => {
    program
        .command('read')
        .description("Deserializes the avro message bytes (expressed as hex string) from stdin.")
